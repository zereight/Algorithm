...
function dijikstra(지점개수, 출발지점, 도착지점, 경로정보) {
  const 출발노드 = 출발지점;
  const 출발지점으로부터의거리Array = Array.from(
    { length: 지점개수 },
    () => Infinity
  );
  const visited = Array.from({ length: 지점개수 }, () => false);

  출발지점으로부터의거리Array[출발노드] = 0;
  visited[출발노드] = true;

  const pq = new PriorityQueue();

  경로정보[출발지점].forEach(([지점, 비용]) => {
    const 우선순위 = 비용;
  ...
  while (!pq.isEmpty()) {
    ...
    const 현재시점가장가까운지점 = node.element;

    if (visited[현재시점가장가까운지점]) continue;
    visited[현재시점가장가까운지점] = true;

    경로정보[현재시점가장가까운지점].forEach(([목적지, 비용]) => {
      if (
        출발지점으로부터의거리Array[현재시점가장가까운지점] >=
        출발지점으로부터의거리Array[목적지] + 비용
      ) {
        출발지점으로부터의거리Array[현재시점가장가까운지점] =
          출발지점으로부터의거리Array[목적지] + 비용;

        pq.enqueue(목적지, 출발지점으로부터의거리Array[현재시점가장가까운지점]);
      }
    });
  }

  return 출발지점으로부터의거리Array[도착지점];
}
